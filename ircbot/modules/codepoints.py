...
def u(ircbot, input):
    ...
    arg = input.bytes[3:]
    if not arg:
        return ircbot.reply('You gave me zero length input.')
    elif not arg.strip(' '):
        if len(arg) > 1:
            return ircbot.reply('%s SPACEs (U+0020)' % len(arg))
        return ircbot.reply('1 SPACE (U+0020)')
    ...
    if printable:
        ...
        if extended:
            ...
            if result is not None:
                ...
                ircbot.reply("Sorry, no results for %r." % arg)
    else:
        ...
        # look up more than three podecoints
            ...
            ircbot.reply(' '.join('U+%04X' % ord(c) for c in text))
        else:
            ircbot.reply('Sorry, your input is too long!')
u.commands = ['u']
u.example = '.u 203D'

def bytes(ircbot, input):
    """Show the input as pretty printed bytes."""
    b = input.bytes
    ircbot.reply('%r' % b[b.find(' ') + 1:])
