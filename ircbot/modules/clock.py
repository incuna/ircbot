...
def yi(ircbot, input):
    ...
    if extraraels == 4:
        return ircbot.say('Yes! PARTAI!')
    else:
        ircbot.say('Not yet...')
yi.commands = ['yi']
yi.priority = 'low'

def tock(ircbot, input):
    """Shows the time from the USNO's atomic clock."""
    u = urllib.urlopen('http://tycho.usno.navy.mil/cgi-bin/timer.pl')
    info = u.info()
    u.close()
    ircbot.say('"' + info['Date'] + '" - tycho.usno.navy.mil')
tock.commands = ['tock']
tock.priority = 'high'

def npl(ircbot, input):
    ...
    if data:
        ...
        ircbot.say(result + ' - ntp1.npl.co.uk')
    else:
        ircbot.say('No data received, sorry')
npl.commands = ['npl']
npl.priority = 'high'
