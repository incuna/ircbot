...
def head(ircbot, input):
    ...
    if not uri and hasattr(ircbot, 'last_seen_uri'):
        try:
            uri = ircbot.last_seen_uri[input.sender]
        except KeyError:
            return ircbot.say('?')
    ...
    try:
        ...
        return ircbot.say("Can't connect to %s" % uri)
    except httplib.InvalidURL:
        return ircbot.say("Not a valid URI, sorry.")

    if not isinstance(info, list):
        try:
            info = dict(info)
        except TypeError:
            return ircbot.reply('Try .head http://example.org/ [optional header]')
        info['Status'] = '200'
    else:
        newInfo = dict(info[0])
    ...
    if header is None:
        ...
        ircbot.reply(', '.join(data))
    else:
        headerlower = header.lower()
        if info.has_key(headerlower):
            ircbot.say(header + ': ' + info.get(headerlower))
        else:
